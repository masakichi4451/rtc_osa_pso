...
%  We set the capacity of each server node $s\in V_{\rm S}$,  $M_s$, to one to complete the D-OSA-PSO instance.  We set $\min(H_{\rm APL},H_s)$ to one for all servers $s \in V_{\rm{S}}$.}
  ...
  We can assign each user node to the same server node as in the case where server node $V_{\ rmZ}$ fails, which also satisfies the maximum status holding time constraint.
