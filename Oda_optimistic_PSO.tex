...
OSA allows servers to process events without prior order enforcement, rolling back their status to the time of the event occurrence if an out-of-order event is detected, \blue{which can improve delay performance compared to CSA.}
...
Equation~\eqref{OSA-PSO_l} ensures that at least one user is assigned to each \blue{used} server in each failure scenario. Equations~\eqref{OSA-PSO_m} and~\eqref{OSA-PSO_n} express the range of decision variables $x_{ijf}$ and $y_{sf}$, respectively.
...
\blue{Regardless of the number of users or the network topology, increasing the permissible value of $\min(H_{\rm APL}, H_s)$ allows the user that determines the maximum value of $x_{usf}d(u,s) + x_{stf}d(s,t)$ to select a closer server, thereby reducing $2D_{\mathrm{U}f}$.}
