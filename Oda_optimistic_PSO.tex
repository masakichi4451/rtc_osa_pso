...
OSA allows servers to process events without prior order enforcement, rolling back their status to the time of the event occurrence if an out-of-order event is detected, \magenta{which improves delay performance compared to CSA.}
...
Equation~\eqref{OSA-PSO_l} ensures that at least one user is assigned to \magenta{a server used in each failure scenario.} Equations~\eqref{OSA-PSO_m} and~\eqref{OSA-PSO_n} express the range of decision variables $x_{ijf}$ and $y_{sf}$, respectively.
...
In such cases, to satisfy~\eqref{OSA-PSO_g}, %\blue{a user $u$ must select a server $s$ that reduces the delay to the farthest server $t$, $x_{usf}d(u,s) + x_{stf}d(s,t)$, even if it results in a slight increase in the user-server delay $x_{usf}d(u,s)$}.
\magenta{a user~$u$ that contributes to the objective value in~\eqref{OSA-PSO_a} must select a more distant server $s$ to minimize it, which results in a slight increase in the user-server delay $x_{usf}d(u,s)$.}
...
\magenta{Regardless of the number of users or the network topology, increasing an admissible value for $\min(H_{\rm APL}, H_s)$ allows the user that determines the maximum value of $x_{usf}d(u,s) + x_{stf}d(s,t)$ to select a closer server, thereby reducing $2D_{\mathrm{U}f}$.}
