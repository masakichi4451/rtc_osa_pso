...
\section{Proposed model: CMND-PSO}
\label{sec:proposed_mode_cmnd_pso}

\subsection{Problem definitions}

We describe the problem definition of CMND-PSO. Consider a network described as an undirected graph $G = (V, E)$.
Let $V$ be the set of nodes and $E$ be the set of edges.
Let $V_{\rm{U}} \subseteq V$ and $V_{\rm{S}} = \{1, \cdots, |V_{\rm{S}}|\} \subseteq V$ denote the set of users and the set of potential server locations, where $|V_{\rm{S}}| \geq 2$, respectively.
We assume that each server has sufficient computing capacity, i.e., there is no limit on the number of users that can be accommodated by each server.
A user is denoted by $u \in V_{\rm{U}}$, and a server is denoted by $s \in V_{\rm{S}}$.
It is noted that $V_{\rm{U}} \cup V_{\rm{S}} = V$ and $V_{\rm{U}} \cap V_{\rm{S}} = \emptyset$.
Let $E_{\rm{U}} \subseteq E$ and $E_{\rm{S}} \subseteq E$ denote the set of edges connecting users and servers and the set of edges connecting servers, respectively.
A edge between user~$u \in V_{\rm{U}}$ and server~$s \in V_{\rm{S}}$ is denoted by $(u,s) \in E_{\rm{U}}$.
A edge between server~$s \in V_{\rm{S}}$ and server~$t \in V_{\rm{S}} \setminus \{s\}$ is denoted by $(s,t) \in E_{\rm{S}}$.
It is noted that  $E_{\rm U} \cup E_{\rm S} =E$ and $E_{\rm{U}} \cap E_{\rm{S}} = \emptyset$.
This paper considers that at most one server fails at the same time.
Let $F = \{0, 1, \cdots, |V_{\rm{S}}|\}$ denote the set of failure scenarios, where $f = 0 \in F$ indicates the scenario where no server fails and $f \in F \setminus \{0\}$ represents the scenario where server~$f$ fails.
We assume that each server has sufficient computing capacity, i.e., there is no limit on the number of users that can be accommodated by each server.

Let $d(u,s)$ and $d(s,t)$ denote the delay between user~$u \in V_{\rm{U}}$ and server~$s \in V_{\rm{S}}$ and the delay between server~$s \in V_{\rm{S}}$ and server~$t \in V_{\rm{S}} \setminus \{s\}$, respectively.
We assume that $d(u,s) \geq 0$, $d(u,s) = d(s,u)$.
Similarly, we assume that $d(s,t) \geq 0$, $d(s,t) = d(t,s)$, and $d(s,s) = 0$.
We also assume that the delays between servers satisfy the triangle inequality, i.e., $d(s_1,s_2) + d(s_2,s_3) \geq d(s_1,s_3)$ for every triple of elements $s_1, s_2, s_3 \in V_{\rm{S}}$.
% この後，Oda_CMNDと同じように，査読後に追加した部分とユーザには三角不等式仮定しなくていいことを述べるべき？
Let $y_{sf}$ be a binary decision variable that takes one if server~$s \in V_{\rm{S}}$ is selected under failure scenario~$f \in F$ and zero otherwise.
Let $x_{ijf}$ be a binary decision variable that takes one if edge $(i,j) \in E$ is used under failure scenario~$f \in F$ and zero otherwise.
Let $z_{stf}$ be a binary decision variable that takes one if server~$s \in V_{\rm{S}}$ selects server~$t \in V_{\rm{S}} \setminus \{s\}$ as a secondary server under failure scenario~$f \in F$ and zero otherwise.
It is clear that $x_{usf} \le y_{sf}$ for each pair of user~$u \in V_{\rm{U}}$ and server~$s \in V_{\rm{S}}$ under each failure scenario~$f \in F$.
It is also apparent that $x_{stf} \le y_{sf}$ and $x_{stf} \le y_{tf}$ for each pair of servers $s \in V_{\rm{S}}$ and $t \in V_{\rm{S}} \setminus \{s\}$ under each failure scenario~$f \in F$.
Let $D_{\mathrm{U}f}$ and $D_{\mathrm{S}f}$ denote the maximum values of $d(u,s)$ and $d(s,t)$, respectively, where $(u,s) \in E_{\rm{U}}$ and $(s,t) \in E_{\rm{S}}$ are used under failure scenario~$f \in F$.
Let $L_f$ represent the maximum delay between each used server and the corresponding secondary servers under failure scenario~$f \in F$.
Let $D$ denote the largest total delay of $2D_{\mathrm{U}f}+D_{\mathrm{S}f}+L_f$ among all failure scenarios.
We define CMND-PSO as an optimization problem to minimize $D$.
% 
\begin{table}[tb]
    \centering
    ...
    \begin{tabular}{p{1.3cm}|cp{5.4cm}}
        ...
        & $F$ & Set of failure scenarios, where $f = 0 \in F$ indicates the scenario where no server fails and $f \in F \setminus \{0\}$ represents the scenario where server~$f$ fails. \\
        & \blue{$H_{\rm APL}$} & \red{Maximum status holding time of an aplication.} \\
        & \blue{$H_s$} & \red{Maximum status holding time of server $s$.} \\ \hline
        Decision parameters & $D$ & The largest total delay of \blue{$2D_{\mathrm{U}f}$} among all failure scenarios. \\
        & $D_{\mathrm{U}f}$ & Maximum value of $d(u,s)$, where $(u,s) \in E_{\rm{U}}$ is used under failure scenario~$f \in F$. \\
    ...
    \end{tabular}
\end{table}
Table~\ref{tbl:notation} shows the given parameters and decision parameters of the problem.
...
% \vspace{-0.3cm}
We explain the ILP problem in~\eqref{SFNC a}--\eqref{SFNC v}.
Equation~\eqref{SFNC a} is the objective function, which minimizes the largest total delay among all failure scenarios.
Equation~\eqref{SFNC b} states that each user is assigned to a server that does not fail under each failure scenario.
Equation~\eqref{SFNC d} indicates that a failed server cannot host any user under each failure scenario.
Equation~\eqref{SFNC e} calculates $D_{\mathrm{U}f}$, which is the maximum delay between connected users and servers under failure scenario~$f$.
% $D_{\mathrm{U}f}$を定義したときに説明しているなら，which 以降は不要？→不要なら，\eqref{SFNC e}~\eqref{SFNC g}はそれぞれ~~を計算するって言うだけにする．
Equation~\eqref{SFNC f} calculates $D_{\mathrm{S}f}$, which is the maximum delay between connected servers under failure scenario~$f$.
Equation~\eqref{SFNC g} calculates $L_f$, which is the maximum delay between the main and secondary servers under failure scenario~$f$.
Equation~\eqref{SFNC h} computes the largest total delay of $2D_{\mathrm{U}f}+D_{\mathrm{S}f}+L_f$ among all failure scenarios.
% あるサーバがあるfailure scenarioにおいて故障していない場合，どのサーバも故障していないscenarioにおいてそのサーバに接続していたユーザはそのサーバに接続し続ける．
Equation~\eqref{SFNC i} states that if a server does not fail in a failure scenario, the users connected to the server in a scenario where no server fails continue to be connected to the server.
Equation~\eqref{SFNC k} expresses that if a server accommodates one user, the server is used.
Equations~\eqref{SFNC l}--\eqref{SFNC n} express $x_{stf} = y_{sf} \cdot y_{tf}$.
% データの一貫性の確認のため，選ばれるサーバが少なくとも$N_{\rm{SS}} + 1$台であることを保証する．
Equation~\eqref{SFNC o} indicates that in each failure scenario, at least $N_{\rm{SS}} + 1$ servers are selected to ensure data consistency. 
Equation~\eqref{SFNC p} states that if an edge between servers $s$ and $t$ forms a main-secondary server pair, the edge is selected in failure scenario~$f$ satisfying $x_{stf} = 1$.
Equation~\eqref{SFNC q} ensures that each selected surviving server selects $N_{\rm{SS}}$ secondary servers under each failure scenario.
% 故障サーバはmain server, secondary serverとして選ばれない
Equations~\eqref{SFNC r} and~\eqref{SFNC s} indicate that a failed server is neither selected as a main nor secondary server under each failure scenario.
