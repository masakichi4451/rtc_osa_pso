...
Distributed server processing is effective for this purpose, and there are two synchronization algorithms: conservative synchronization algorithm (CSA) and optimistic synchronization algorithm (OSA).
...
This paper proposes a OSA-based server allocation model for delay-sensitive applications with preventive start-time optimization (PSO) under single-server failures.
The proposed model minimized the largest total delay across all failure scenarios while satisfying the maximum status holding time constraints with PSO under single-server failures.
...
Numerical results also demonstrate that the proposed model achieves a lower largest total delay than start-time optimization and provides greater stability by preventing unnecessary user reassignments compared to run-time optimization.
}
\end{abstract}
% \vspace{-0.3cm}
\begin{IEEEkeywords}
\blue{Server allocation, optimistic synchronization algorithm, preventive start-time optimization, server failure.}
\end{IEEEkeywords}
...
In contrast, OSA allows events to be processed without prior order enforcement; if an out-of-order event is detected, the system performs a rollback to guarantees the event order and corrects its results.
...
\blue{
The work~\cite{OSA_Kawabata_2021} introduced an DSP model with OSA.
When a server receives an event from its assigned user, it rolls back its status to the time when the event was occurred and processes the event and sends the result back to the user immediately.
...
In this context, start-time optimization (SO) reallocates only users connected to failed server to minimize the total delay in each failure scenario. 
...
PSO effectively minimizes the largest total delay accross all failure scenarios while avoiding unnecessary service interruptions.
}
...
\blue{
This paper proposes a OSA-based server allocation model with PSO against single-server failures.
...
When $\min(H_{\rm APL}, H_s) \geq 14$, allocation~1 is preferable to allocation~2; $s$ used in $H_s$ is applied for all servers in the network.
...
\begin{figure}[tb]
  ...
  \end{center}
    % \vspace{-0.4cm}
  \caption{\blue{Example of server allocations with OSA.}}
...
\begin{figure}[tb]
  ...
  \end{center}
    % \vspace{-0.4cm}
  \caption{\blue{Example of CSA-based server allocation with SO, RO, and PSO.}}
...
Let $f$ denotes the failed server.
...
For example, under RO, user~3 may be assigned to server~2 when $f = 0$, but reassigned to server~3 when $f = 1$, despite server~2 not having failed.
...
\blue{
We poposed the optimistic synchronization-based server allocation model with preventive start-time optimization (OSA-PSO) against single-server failures.
...
We conpared the proposed OSA-PSO model with the conventional conservative synchronization-based server allocation model with PSO (CSA-PSO) and showed that OSA-PSO achieves a lower delay than CSA-PSO when the maximum status holding time is large.
We also compared the proposed OSA-PSO model with two baselines, start-time optimization (SO) and run-time optimization (RO).
The results showed that the proposed model outperforms SO by jointly optimizing across all failure scenarios, achieving an average improvement of 0.10 in the largest total delay and up to 0.41 in the worst case.
The results also showed that the proposed model achieves a largest total delay nearly equivalent to that of RO, while completely eliminating instability due to unnecessary user reassignments, which RO causes in 41\% of users per failure scenario.
}
